 ...
 *   --save                     Save results to $X_RESEARCH_OUTPUT_DIR or ./drafts
...
const OUTPUT_DIR_ENV = "X_RESEARCH_OUTPUT_DIR";

function getOutputDir(): string {
  const configured = process.env[OUTPUT_DIR_ENV]?.trim();
  if (!configured) return join(process.cwd(), "drafts");
  if (isAbsolute(configured)) return configured;
  return resolve(process.cwd(), configured);
}

const DRAFTS_DIR = getOutputDir();
